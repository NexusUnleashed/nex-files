...
## handler.process(current_line)

Evaluates one line against all enabled reflexes.

```js
nexAction.triggers.process(nexusclient.current_line);
```

`current_line` must provide a `parsed_line` object with `text()` and `chunks` (foreground/background colors). Example adapter:

```js
const line = {
  parsed_line: {
    chunks: [{ _fg: "#ffffff", _bg: "#000000" }],
    text() {
...
nexAction.triggers.process(line);
```
